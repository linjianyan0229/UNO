...
// 判断某玩家是否为房主
export function isRoomOwner(roomInfo: RoomInfo, player: PlayerInfo) {
  return roomInfo.owner.id === player.id && roomInfo.owner.name === player.name;
}

// 根据 socket 找到其所在房间
export function findRoomBySocket(ws: WebSocket.WebSocket) {
  for (const [roomCode, roomInfo] of roomCollection) {
    if (roomInfo.players.some((player) => player.socketInstance === ws)) {
      return { roomCode, roomInfo };
    }
  }
  return undefined;
}
...
export function createAIPlayer(difficulty: AIDifficulty): PlayerInfo {
  const suffix = Math.random().toString(36).slice(2, 8).toUpperCase()
  return {
    id: `ai-${suffix}`,
    name: difficulty === 'hard' ? '战术AI' : difficulty === 'easy' ? '训练AI' : 'UNO AI',
    socketInstance: AI_SOCKET,
    cards: [],
    uno: false,
    ready: true,
    avatar: '',
